...
class RookTest {
    ...
    void moveTest1(int startLine, int startColumn, int endLine, int endColumn, boolean canMove) {
        ...
        board.getBoard()[0][1] = new Rook(WHITE);
        board.getBoard()[1][0] = new Rook(WHITE);
        board.getBoard()[1][1] = new Rook(WHITE);

        assertEquals(canMove, board.moveToPosition(startLine, startColumn, endLine, endColumn));
    }

    @ParameterizedTest
    @CsvSource({"0,0,0,0,false", "10,0,0,0,false", "0,0,7,0,true", "0,0,0,7,false", "0,0,5,5,false"})
